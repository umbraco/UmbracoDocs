...
Register the Worker as a public OAuth client with PKCE. See [Umbraco Setup](umbraco-setup.md) for the full Composer code. The redirect URI for each project must include the Cloud alias:
...
See [External Login Short Circuit](external-login-short-circuit.md) for the full setup.
...
**Cause**: The Cloud project is missing the [External Login Short Circuit](external-login-short-circuit.md) composer, or `Umbraco.Cloud.Cms` is not installed.
