...
{% hint style="info" %}
Copilot Workspace complements [Copilot](../agent-copilot/copilot.md) (referred to in these docs as **Contextual Copilot**), the existing docked sidebar scoped to the item you're editing. Both ship under the same **Copilot** brand in the backoffice -- the section itself is labeled **Copilot**, with a **Workspace** dashboard sub-label. Use Copilot for quick, in-context help while editing; use Copilot Workspace for broader, cross-site tasks, or when you want a durable, searchable history of your conversations with AI.
{% endhint %}
...
{% hint style="info" %}
Copilot Workspace stores conversations and projects in its own database tables (migration prefix `UmbracoAIConversations_`), unlike the existing Copilot sidebar, which has no database of its own. Migrations run automatically on application startup.
{% endhint %}
...
- **AG-UI Integration** - Real-time streaming responses, reusing the same chat UI as Copilot
...
The Copilot Workspace surface is registered with `SurfaceId = "copilot-workspace"`. Unlike Copilot, which only shows agents scoped to the current section and entity type, Copilot Workspace availability is gated solely by this surface opt-in -- an agent enabled here is available everywhere in the Workspace.
...
| Section                                          | Description                                  |
| ------------------------------------------------- | --------------------------------------------- |
| [Using Copilot Workspace](copilot-workspace.md)   | Conversations, projects, and the context panel |
