...
If the resolved data can change while the user has the Value Summary open, for example, if a linked entity can be renamed elsewhere in the backoffice, return an `asObservable` function alongside `data`. The collection subscribes to it and updates the displayed values reactively without a full reload.
