...
In order to get a clean instance of Umbraco, follow our installation guide for how to [Install an Umbraco project template](/broken/pages/Dksuk4APBTbLNXEdboGE#install-using-net-cli).
...
{% hint style="info" %}
Umbraco can create an SQL Server database for you during the unattended install process. The user specified by the credentials in your connection string needs to have the `CREATE DATABASE` permission granted and the global setting [InstallMissingDatabase](../../../develop-with-umbraco/configuration/globalsettings.md#install-missing-database) is set to `true`.
...
{% hint style="warning" %}
Do not add `Cache=Shared` to a SQLite connection string. Shared cache causes `database table is locked` errors on the write-ahead logging databases that Umbraco creates. For more information, see the [Connection strings settings](../../../develop-with-umbraco/configuration/connectionstringssettings.md) article.
{% endhint %}
