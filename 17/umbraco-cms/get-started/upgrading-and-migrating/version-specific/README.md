...
_When you need to reference `Umbraco.Cms.DevelopmentMode.Backoffice`_?
...
_When you do not need the package_?
...
The default value of the `UseHttps` configuration in [Global Settings](../../../develop-with-umbraco/configuration/globalsettings.md) has been changed from `false` to `true`.
...
Read more in the article on [custom indexing](../../../develop-with-umbraco/application-code/examine/indexing.md) and see [PR #18988](https://github.com/umbraco/Umbraco-CMS/pull/18988) for reference.
...
More details and code examples can be found in the [Two-Factor Authentication](../../../run-in-production/security/two-factor-authentication.md) article.
...
More details and code examples can be found in the [External Login Providers](../../../run-in-production/security/external-login-providers.md) article.
...
If you are using **TinyMCE** plugins or custom TinyMCE configuration you need to migrate to the latest version. Learn more about this in the [Rich Text Editor documentation](../../../model-your-content/property-editors/built-in-umbraco-property-editors/rich-text-editor/).
...
In Umbraco 15, the internal data format for [Block Editors](../../../model-your-content/property-editors/built-in-umbraco-property-editors/block-editor/) has changed. This causes a content migration to run when upgrading.

This content migration can take a while to complete on a large site, causing it to be unresponsive for the duration. To speed up the migration, it is advised to [clean up old content versions](../../../develop-with-umbraco/configuration/content-version-cleanup.md) before upgrading.
...
Umbraco 8.1 replaces AutoMapper with [UmbracoMapper](../../../extend-your-project/server-side-extensions/mapping.md). This in itself will not break anything on your site. If you have used AutoMapper in your own code you will have to either include the package yourself or switch your implementation to use UmbracoMapper.
