...
1. Install the latest [dotnet templates](/broken/pages/Dksuk4APBTbLNXEdboGE): `dotnet new install Umbraco.Templates`.
...
To render these partial views, we need a new Document Type with a dedicated template (see also [Defining Content](../../model-your-content/content-types-and-structure/data/defining-content/)):
...
We can leverage the [built-in Notifications](../../extend-your-project/server-side-extensions/notifications/) to handle the automatic Member Group assignment. Specifically the `MemberSavedNotification`, which is triggered whenever a Member is saved.
