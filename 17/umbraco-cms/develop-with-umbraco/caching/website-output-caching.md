---
description: >-
  Boost website performance with opt-in server-side output caching for Umbraco
  pages that are rendered with Razor templates.
---
...
{% hint style="info" %}
The Content Delivery API has its own output caching support. For details, see the [Output caching](../headless-and-apis/content-delivery-api/output-caching.md) article in the Delivery API section.
{% endhint %}
...
All standard Umbraco pages are cached. This covers every page routed through the default Umbraco rendering pipeline. Pages rendered by custom controllers that inherit from `RenderController` are also cached (see [Custom MVC controllers (Umbraco Route Hijacking)](../application-code/backend-and-custom-logic/routing/custom-controllers.md)).
...
**Interface:** `IWebsiteOutputCacheRequestFilter` **Registration:** Single — replace the default with `builder.Services.AddUnique<IWebsiteOutputCacheRequestFilter, YourFilter>()`.
...
**Interface:** `IWebsiteOutputCacheDurationProvider` **Registration:** Single — replace the default with `builder.Services.AddUnique<IWebsiteOutputCacheDurationProvider, YourProvider>()`.
...
**Interfaces:** `IWebsiteOutputCacheTagProvider` and `IWebsiteOutputCacheEvictionProvider` **Registration:** Multiple — add with `builder.Services.AddSingleton<>()`. Multiple providers of each type are additive.
...
**Interface:** `IWebsiteOutputCacheManager` **Usage:** Inject via dependency injection. All methods are no-ops when output caching is not enabled.
...
**Interface:** `IWebsiteOutputCacheVaryByProvider` **Registration:** Multiple — add with `builder.Services.AddSingleton<IWebsiteOutputCacheVaryByProvider, YourProvider>()`. Multiple providers are additive.
...
For more details on route hijacking, see the [Custom MVC controllers (Umbraco Route Hijacking)](../application-code/backend-and-custom-logic/routing/custom-controllers.md) article.
