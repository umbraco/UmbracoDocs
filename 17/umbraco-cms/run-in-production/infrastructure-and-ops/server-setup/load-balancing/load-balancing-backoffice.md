...
If you have custom recurring background jobs that should only run on a single server, you'll need to implement `IDistributedBackgroundJob`. See [Scheduling documentation](../../../../extend-your-project/server-side-extensions/scheduling.md#background-jobs-when-load-balancing-the-backoffice) for more information.
...
**Other Environments:** Configure a shared storage location using [the `Umbraco:CMS:Hosting:TemporaryFileUploadLocation` setting.](../../../../develop-with-umbraco/configuration/hostingsettings.md#temporary-file-upload-location)
