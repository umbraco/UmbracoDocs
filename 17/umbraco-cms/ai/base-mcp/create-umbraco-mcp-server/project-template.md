...
* **Configuration** — Mode and slice registries in `src/config/` that control tool filtering. See [Configuration](../sdk/configuration.md) and [Tool Filtering](../sdk/tool-filtering.md) in the SDK docs
* **Testing infrastructure** — Jest setup for both integration tests (against a real Umbraco instance) and LLM eval tests (using the Claude Agent SDK). See [Testing and Evals](../sdk/testing.md) in the SDK docs
...
The `src/umbraco-api/mcp-client.ts` file configures connections to other MCP servers (such as `@umbraco-cms/mcp-dev`). MCP chaining lets you proxy tools from existing servers alongside your custom tools. See [MCP Chaining](../sdk/mcp-chaining.md) in the SDK docs.
