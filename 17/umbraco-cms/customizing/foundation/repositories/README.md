...
A repository must be instantiated where it is used. It should take an [UmbController](../umbraco-controller/) as part of the constructor. This ensures that any contexts consumed in the repository are scoped correctly.

A repository can be initialized directly from an element, but will often be instantiated in a [context](../context-api/), like the Workspace Context.
...
Often, you will find that data is already available and observable in a [context](../contexts/). In that case, subscribing to the context [state](../states.md) will be the right approach to take. This way, you will receive all runtime updates that occur to the data throughout the session.
...
By registering your repository in the [Extension Registry](../../extending-overview/extension-registry/), you make it available to use in different extension kinds that require a repository alias.
