...
6. Add a new remote called origin and pointing to the GitHub clone URL.
...
GitHub is now set up with the needed information to be able to run a deployment back to Umbraco Cloud.

### Allow GitHub to commit to your repository

The sample pipelines have a job called `cloud-sync`. This job is responsible for checking for changes in your Umbraco Cloud project, fetching them, and applying them back to your repository. For this to work, you need to give the `GITHUB_TOKEN` write permissions to the repository during workflow runs.
...
4. Scroll down to the `Workflow permissions` sections.
...
3. Create a folder in the root and call it `.github`.
4. Create two additional folders within `.github`: `workflows` and `powershell`.
5. Copy the 4 YAML files from the `github` folder into the `workflows` folder.
6. Copy the Powershell scripts from the `powershell` folder to the `powershell` folder.
7. Commit the all changes, and push to GitHub.
