---
description: >-
  Choose the right upgrade path and resource strategy to safely manage database
  migrations and prevent downtime on your production environment.
---
...
You should review this guide **after** you have already followed the standard [Major Upgrades](./) guide.
...
Follow the instructions in the [Building the Copy](planning-major-upgrades-safely-on-live.md#building-the-copy) section below.
...
* [Approach 1: Run the upgrade on the existing Cloud plan](planning-major-upgrades-safely-on-live.md#approach-1-run-the-upgrade-on-the-existing-cloud-plan)
...
* [Approach 2: Run a controlled local migration (Local Upgrade Option)](planning-major-upgrades-safely-on-live.md#approach-2-run-a-controlled-local-migration-local-upgrade-option)
...
* [Approach 3: Use dedicated resources for the upgrade window (Cloud Upgrade Option)](planning-major-upgrades-safely-on-live.md#approach-3-use-dedicated-resources-for-the-upgrade-window-cloud-upgrade-option)
...
Many projects can complete the major upgrade successfully on their existing plan. Proceed with the standard approach outlined in the [Execution Summary](planning-major-upgrades-safely-on-live.md#execution-summary-from-pre-upgrade-to-go-live) at the end of this guide.
...
3. **Allowlist your IP address:** Open the **SQL connection details** for the **Live** environment in the Cloud Portal and **allowlist your IP address**. For more information, see the [Project Settings](../../../../build-and-customize-your-solution/set-up-your-project/project-settings/) and [Working with a Cloud database locally](../../../../build-and-customize-your-solution/set-up-your-project/databases/cloud-database/local-database.md) articles.
...
1. **Scale Up Capacity:** Before pushing the upgrade, navigate to the **Management** menu in the Cloud Portal and move your project onto dedicated resources. Note: Moving to dedicated resources changes your plan and billing, so review pricing before switching. For more information, see the [Project Settings](../../../../build-and-customize-your-solution/set-up-your-project/project-settings/) article.
2. **Deploy the Code:** Push the major upgrade to Live. The migration will run natively inside the Cloud region infrastructure, eliminating internet round-trip latency. (If anything fails to complete, the [Local Upgrade Option](planning-major-upgrades-safely-on-live.md#approach-2-run-a-controlled-local-migration-local-upgrade-option) remains available as an immediate fallback).
...
1. Push the major upgrade to Live following [Step 5: Deploy the upgrade](./#step-5-deploy-the-upgrade) in the Major Upgrades guide.
2. **If it completes successfully:** Verify the site on the new major version to finish the upgrade.
3. **If it stalls or fails:** Immediately transition to the [Local Upgrade Option](planning-major-upgrades-safely-on-live.md#approach-2-run-a-controlled-local-migration-local-upgrade-option) to complete the schema migration from your machine.
...
* [Media on Cloud](../../../../build-and-customize-your-solution/handle-deployments-and-environments/media/)
...
* [Project Settings](../../../../build-and-customize-your-solution/set-up-your-project/project-settings/)
