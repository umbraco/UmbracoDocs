---
description: >-
  Configuring trees in Umbraco UI Builder, the backoffice UI builder for
  Umbraco.
---
...
The tree configuration for Umbraco UI Builder sections is a sub-configuration of a [`Section`](../sections/) config builder instance and is accessed via its `Tree` method.
...
The tree configuration for existing sections is a sub-configuration of a [`WithSection`](../sections/#extending-an-existing-section) config builder instance and is accessed via one of its `AddTree` methods.
...
You can extend existing trees adding Umbraco UI Builder context apps and virtual sub trees by calling the `WithTree` method of a [`WithSectionConfigBuilder`](../sections/#extending-an-existing-section) instance.
