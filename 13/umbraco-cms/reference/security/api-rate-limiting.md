...
{% hint style="info" %}
When Umbraco runs behind a WAF or reverse proxy, rate-limiting may fail if the client IP address is not forwarded correctly. Configure your proxy or WAF to send the original client IP using headers like X-Forwarded-For. Ensure Umbraco reads these forwarded headers, or all requests may appear from one IP and cause incorrect rate-limit enforcement.
{% endhint %}