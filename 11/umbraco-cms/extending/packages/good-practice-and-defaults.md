...
For example:
On windows a file might be located at `d:\website\robots.txt` while on linux this might look like `/home/website/robots.txt`
...
Settings can be saved to the database; simple settings can be stored in the databse using the Umbraco `IKeyValueservice`; for more complex setting you might need to use a custom table
