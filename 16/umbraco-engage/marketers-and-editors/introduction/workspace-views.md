---
description: >-
  Discover how the Workspace Views enhance each node with different features for
  improved content management.
---

# Workspace Views

In the Content section of Umbraco, you will see the Umbraco nodes. Most of them will relate to a specific page in your website. If you have installed the Umbraco Engage, each Umbraco node will have three extra Workspace Views.

![Umbraco content nodes with Engage workspace views](<../../.gitbook/assets/Engage-content-apps-v16 (1).png>)

{% hint style="info" %}
Workspace Views are a concept in Umbraco CMS. See the [Workspace Views](https://docs.umbraco.com/umbraco-cms/extend-your-project/backoffice-extensions/extending-overview/extension-types/workspaces/workspace-views) article in the CMS documentation to learn more.
{% endhint %}

## Analytics

If you navigate to a node, you will see the Analytics workspace view. If you open the Workspace View, the Analytics data of this specific node is loaded.

![Viewing the Analytics workspace view for a specific node](<../../.gitbook/assets/Analytics-v16 (1).png>)

## A/B Tests

The A/B Tests workspace view allows you to test your Umbraco node within the splitview functionality. Every Document Type can be A/B tested:

![Using the A/B Testing workspace view to test splitview functionality](<../../.gitbook/assets/AB-Tests-v16 (1).png>)

## Personalization

The Personalization workspace view allows you to score a specific page based on the customer journey and persona (for implicit content scoring).

![Personalization workspace view for scoring a page](<../../.gitbook/assets/Personalization-v16 (1).png>)

It also allows you to set up personalized variants for each node.

![Setting up personalized variants for each Umbraco node](<../../.gitbook/assets/Personalized-variants-v16 (1).png>)

## Managing the access to Workspace Views

In the **Engage** -> **Settings** section, you can manage which Document Types the workspace views are displayed, and which Umbraco user groups can access them. They can be managed per Document Type and per user group.

![Document Type Permissions in the Settings section](<../../.gitbook/assets/engage-content-apps-doc-type-permissions (1).png>)

![User group Permissions in the Settings section](<../../.gitbook/assets/engage-content-apps-user-group-permissions (1).png>)
