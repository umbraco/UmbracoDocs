...
If the title doesn't change colour, you may be running the site without debugging and therefore client dependency will be serving the javascript and css in one file which gets cached. To clear that cached file and get it to load in the new javascript and styles, you need to increment the the version number in the ClientDependency.config file and press save. Now you can reload the page and see the colourful title.
