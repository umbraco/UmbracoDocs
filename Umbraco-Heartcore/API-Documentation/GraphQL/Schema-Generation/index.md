...
A Connection and an Edge type will also be generated
...
type ContentConnection {
  ...
  #  rather than querying for "{ edges { node }
 }
" when no edge data is needed, this field can be used instead.
  #  Note that when clients like Relay need to fetch the "cursor" field on the edge to enable efficient pagination,
  #  this shortcut cannot be used, "and the full "{ edges { node }
 }
