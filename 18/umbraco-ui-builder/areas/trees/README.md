...
The tree configuration for Umbraco UI Builder sections is part of the [`Section`](../sections/) config builder and is accessed via its `Tree` method.
...
To add a tree to an existing section, use one of the `AddTree` methods from the [`WithSection`](../sections/#extending-an-existing-section) config builder.
...
To extend existing trees, call the `WithTree` method on a [`WithSectionConfigBuilder`](../sections/#extending-an-existing-section) instance.
