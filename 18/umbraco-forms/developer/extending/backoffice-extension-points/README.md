...
Before you start, install and configure the `@umbraco-forms/backoffice` package as described in the [Extending](../#extending-the-backoffice) article.
...
Server-side providers, such as field types and workflow types, are registered in C#. These are covered in [Adding A Type To The Provider Model](../adding-a-type/).
...
For full examples of field and setting editors, see [Adding A Field Type To Umbraco Forms](../adding-a-fieldtype/) and [Adding A Workflow Type To Umbraco Forms](../adding-a-workflowtype.md).
...
{% hint style="info" %}
The manifest `alias` must match the `PreviewView` property on your C# field type. This is how Forms knows which preview to render. See [Adding A Field Type To Umbraco Forms](../adding-a-fieldtype/) for the full example.
{% endhint %}
...
For more on settings and converters, see [Adding A Field Type To Umbraco Forms](../adding-a-fieldtype/).
