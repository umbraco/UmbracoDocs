...
Field settings work the same way as settings on any other provider type. See [Adding settings to a type](../adding-a-type/#adding-settings-to-a-type) for the underlying mechanism. See [Setting Types](../adding-a-type/setting-types.md) for the full list of built-in Views, all available `Setting` attribute properties, translations, default values, and inheritance behavior.
