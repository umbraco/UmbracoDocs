...
When you haven't opened a conversation yet, the sidebar and a launcher are shown, letting you start a new chat or pick up a recent one:
...
While chatting, the context panel on the right shows what's grounding the current conversation -- the project's instructions, contexts, and resources (if the conversation belongs to a project), plus anything attached directly to the conversation itself. Collapse it for more chat space, or expand it to review or edit what the agent can see:
