...
* Add OpenAPI documents for the Deploy Environment API and Deploy Schema API to the OpenAPI UI at `/umbraco/openapi`, next to the Deploy Management API. The Extract and StatusReport (`umbracodeploy`) routes are documented in lowercase. The URLs stay case-insensitive.
