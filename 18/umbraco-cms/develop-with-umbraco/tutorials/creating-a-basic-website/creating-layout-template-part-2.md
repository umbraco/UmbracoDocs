...
Let's create a page to display our contact details. For added functionality, you might consider replacing this page with a fully-fledged "Contact Us" form.
...
* **Build Your Own Contact Form (for developers):** If you prefer a custom solution, you can build your own contact form using [Surface Controllers](../../application-code/backend-and-custom-logic/creating-forms.md) in Umbraco.
...
Now let's update the Document Type permissions to specifically add child nodes under the root content node.
...
3. Open the **Simple Content Page** template.
...
You may notice that the footer is now empty. The footer content from the Homepage node is not rendered.
...
5. Click **Insert** and select **Value**.
6. Click **Submit**.
7. Select **Document Type** from the **Choose field** drop-down list.
8. Select **Home Page**.
9. Click **Choose**.
10. Select **footerText** field from the **HomePage** drop-down list.
11. Select **Yes, make it recursive** checkbox. This notifies Umbraco to look up the content tree if the field doesn't exist at the node level for the page requested.
12. Click **Submit**.
13. Click **Save**.
