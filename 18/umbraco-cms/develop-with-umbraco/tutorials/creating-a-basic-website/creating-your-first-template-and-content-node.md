...
3.  Open the template. It contains a little bit of Razor code.
    ...
    * `@using Umbraco.Cms.Web.Common.PublishedModels;` - Imports the namespace containing Umbraco's strongly-typed content models (generated by ModelsBuilder).
    * `@inherits Umbraco.Cms.Web.Common.Views.UmbracoViewPage<ContentModels.HomePage>` - Sets the view to inherit from Umbraco's base view class, strongly typed to the `HomePage` model. This gives `Model` compile-time typed access to `HomePage`'s properties instead of the generic `IPublishedContent`.
    * `@using ContentModels = Umbraco.Cms.Web.Common.PublishedModels;` - Creates the `ContentModels` alias used above in the `@inherits` directive, so you can write `ContentModels.HomePage` instead of the full namespace.
...
There is now a _Template_ in place. That's two out of the three stages complete for the first page.
...
5. Enter the **Name** for the content node. Let's call this _Homepage_.
...
| Footer Text | Copyright Widgets Ltd 2026                                                                                                                                                                                                                                                                                                                           |
...
The default Umbraco page is gone and you will see a basic unstyled page.

{% hint style="info" %}
If you see a blank page, check if the template is entered and remember to save it.
{% endhint %}

![An Unstyled Homepage](../../../.gitbook/assets/unstyled-homepage-after-templateaddition.png)
