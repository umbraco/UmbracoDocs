...
It is possible to list the redirect URLs via the _Redirect Url Management_ dashboard in the _Content_ section. This dashboard lists the original URL, new URL, and culture. It also allows you to delete a URL redirect.
...
Anytime a document is published and its corresponding _url segment_ changes, Umbraco checks its URL (and all its descendants' URLs) for changes. For every URL that has changed, it creates (or updates) a row in the `umbracoRedirectUrl` table. Rows in this table contain: the old url, the create date, and the target content identifier, culture, and a url hash.
