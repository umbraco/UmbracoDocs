...
ChatGPT from OpenAI supports connecting to external MCP servers through its **plugins** settings. ChatGPT runs in the browser, not on your machine. You add the Umbraco MCP server as a plugin through ChatGPT's own settings — there's no local config file to edit.
...
{% hint style="warning" %}
OpenAI updates this UI from time to time, so exact menu names and steps may differ from what's shown below. The destination is always the same: a place to add a plugin by URL. See [OpenAI's Help Center](https://help.openai.com) for the current steps if this doesn't match what you see.
{% endhint %}

## Adding the Plugin

1. Open ChatGPT and go to its plugin settings.
...
4. Save the plugin. A browser window opens for you to authenticate using your Umbraco backoffice credentials.
