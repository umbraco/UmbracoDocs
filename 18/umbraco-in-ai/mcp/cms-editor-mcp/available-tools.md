...
- `create-and-publish-page` — Create a new content page and publish it in one atomic operation — mirrors clicking **Save and publish** (instead of **Save**) on the create screen. Use this as a starting point: pass `name`, `documentTypeId`, an optional `parentId`, and at most a small number of initial values. If the Document Type requires properties this doesn't set, or values fail validation, the whole operation fails and no page is created — use `create-page` followed by `save-and-publish` instead when you need to build up content across multiple calls before publishing.
...
Browse, view, edit (including blocks), publish, and organize reusable Library elements. Elements are the document-like reusable content items that live in the Library section.
