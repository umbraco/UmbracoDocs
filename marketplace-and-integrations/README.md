...
We aim to make all commercial products available as new major versions in alignment with the releases of Umbraco CMS. A new major version of Umbraco, running on the latest version of the .NET framework, is expected every six months. At or around the same time, a compatible Forms, Deploy, Workflow and Commerce will also be made available, along with [the additional, dependent packages](./commercial-product-dependencies.md).
