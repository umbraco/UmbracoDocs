...
- document IDs passed as paramereters to the macro, which leads to another document and its data and so on.
...
Common thing, a data type stores a node ID, but courier doesnt know it, so it cannot add the document as a dependency, and it cannot convert it into
...
Courier doesnt know about it, so cant deploy it. You can write your own provider for it, but this provides you with overhead, and it would
...
it will die, you can increase the request limit and so on, but it wont ever be 100% solid to do, its better to zip your
...
unnneeded overhead to your deployment. Courier adds a lot of extra data and overhead, because it needs to convert to a format that can be transfered and
