...
Surface controller actions can be asynchronous. However, the common naming convention for asynchronous methods (using an `Async` suffix for the action name) will not work by default, due to the inner workings of ASP.NET Core MVC. 
