...
Inside a [custom Controller](broken-reference) access is provided to Services via the `Services` property ([ServiceContext](broken-reference)) and the `UmbracoHelper` via the `Umbraco` property ([UmbracoHelper](../../reference/querying/umbracohelper.md)).
