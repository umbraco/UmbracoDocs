...
* `GetDeliveryApiPropertyValueType()`: This method defines the data type of the custom property value for the Delivery API response. The value type for the Delivery API may differ from the type used for Razor rendering _(specified by `GetPropertyValueType()`)_.
  * In the given example, having just the `Guid` as the value is sufficient for the Razor rendering. Since Razor provides full access to the published content within its rendering context, which is not the case with the Content Delivery API.
